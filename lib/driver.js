...
class XCUITestDriver extends BaseDriver {
  ...
  async startWda (sessionId, realDevice) {
    ...
    await retryInterval(startupRetries, startupRetryInterval, async () => {
      ...
      await this.killPID(this.wda.wdaLocalPort);
  ...
  async killPID (wdaLocalPort) {
    const {stdout} = await exec('bash', ['-c', `ps ax | grep ${wdaLocalPort}`],  {timeout: 300});
    let pid = stdout.split("\n");
    try {
      for (let i=0; i < pid.length; i++) {
        log.debug("Killing webdriveragent process with ID " + pid[i].match(/\d{1,}/g)[0]);
        await exec('kill', ['-9', pid[i].match(/\d{1,}/g)[0]],  {timeout: 300});
      }
    } catch (err) {
      log.debug("Failed to kill process, continuing......");
    }
  }
  ...
  async killAppCurl (Khost) {
    let sessionId = await this.getSessionStatus(Khost);
    let status;
    if (sessionId !== '') {
      const {stdout} = await exec('curl', ['-X', 'DELETE', '-H', 'Content-Type: application/json', url.parse(`${Khost}session/${sessionId}`).href, '--max-time', '2'],  {timeout: 5000});
      status = JSON.parse(stdout).status;
    }
    if (status !== 0) {
      log.info(`This Session with ID ${sessionId} already deleted`);
    } else if (status === 0) {
      log.info("Removing Session and Killing Application");
    }
  }

  async killSession (Khost) {
    await this.killAppCurl(Khost);
    let sessionId = await this.getSessionStatus(Khost);
    if (sessionId !== '') {
      log.debug("Hanging Session ID: " + sessionId);
      const {stdout} = await exec('curl', ['-X', 'GET', '-H', 'Content-Type: application/json', url.parse(`${Khost}session/${sessionId}`).href, '--max-time', '2'],  {timeout: 5000});
      log.debug(JSON.stringify(stdout));
      let pid = JSON.parse(stdout).value.capabilities.CFBundleIdentifier;
      log.debug("Hanging Hackpium Process ID " + pid.replace("local.pid.", ""));
      await exec('kill', ['-9', pid.replace("local.pid.", "")],  {timeout: 5000});
    }
  }

  async getSessionStatus (Khost) {
    let sessionId = '';
    let out, err;
    try {
      const {stdout, stderr} = await exec('curl', ['-X', 'GET', '-H', 'Content-Type: application/json', url.parse(`${Khost}status`).href, '--max-time', '2'],  {timeout: 5000});
      out = stdout;
      err = stderr;
      log.debug("Status WDA" + JSON.stringify(out));
      log.debug("Error Output" + JSON.stringify(err));
    } catch (e) {
      out = e.stdout;
      err = e.stderr;
    } finally {
  ...
  async shutdownSimulator () {
    let shutdown = [this.opts.udid, 'shutdown'];
    log.info(`Simulator with udid '${this.opts.udid}' already booted and need to take it down`);
    await exec('fbsimctl', shutdown, {timeout: 50000});
  }
