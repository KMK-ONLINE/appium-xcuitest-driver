...
class WebDriverAgent {
  ...
  async uninstall () {
    log.debug(`we don't need to remove WDA application from device`);
  ...
  getXcodeBuildCommand (buildOnly = false) {
    ...
    if (this.xcodeVersion.major < 8) {
      ...
      if (this.usePrebuiltWDA) {
        cmd = 'fbsimctl';
        args = [this.device.udid, 'launch_xctest'];
      } else {
    ...
    if (!this.usePrebuiltWDA) {
      ...
      genericArgs = [
        '/Users/woi-te/Library/Developer/Xcode/DerivedData/WebDriverAgent-dikkwtrisltbeobjmfvpthwwekvs/Build/Products/Debug-iphonesimulator/WebDriverAgentRunner-Runner.app/PlugIns/WebDriverAgentRunner.xctest/',
        'com.apple.test.WebDriverAgentRunner-Runner',
        '--port', this.wdaLocalPort,
        '-listen'
      ];
    }
  ...
  async createXcodeBuildSubProcess (buildOnly = false) {
    ...
    log.debug(`Beginning with command '${cmd} ${args.join(' ')}' ` +
    ...
    xcodebuild.on('output', (stdout, stderr) => {
      ...
      if (out.indexOf('Writing diagnostic log for test session to') !== -1) {
        ...
        log.debug(`Log file for xcodebuild test:`);
      }
